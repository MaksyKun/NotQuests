        ...
        commandManager.command(builder.literal("create", Description.of("Create a new quest."))
                ...
                .handler((context) -> {
                    if (context.flags().contains(notQuests.getCommandManager().categoryFlag)) {
                        ...
                        context.sender().sendMessage(notQuests.parse(notQuests.getQuestManager().createQuest(context.get("quest-name"), category)));
                    } else {
                        context.sender().sendMessage(notQuests.parse(notQuests.getQuestManager().createQuest(context.get("quest-name"))));
                    }
                }));
    }
}
